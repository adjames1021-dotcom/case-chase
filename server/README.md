...
- **No bot accounts.** Before signing up, the game fetches a challenge from `/api/challenge` and works out a matching answer (a small proof of work: about a second of the browser's time, done while the player types). The server checks the answer, and each challenge works for one account only. Sign-ups are also refused if the form's hidden "website" field is filled in (people never see it; bots fill in every field), or if the form comes back quicker than a person could type. On top of that, each network can make 20 accounts an hour and 60 a day, each device 5 a day, and the whole game takes at most 300 new accounts an hour. **A device that makes a second new account within a minute is blocked** (no more sign-ups or logins from it), and its browser can't make new accounts for a day; admins lift either from the panel. **A random pause between new accounts** (off by default; set a range like 10–40 seconds in the admin Game tab) applies to everyone, so a bot can't make accounts quickly however it spoofs its device or address — accounts an admin creates in the panel skip it. The name filter also reads a lower-case l as an i (the NlGGER trick).

- **CAPTCHA (Cloudflare Turnstile), optional.** When the environment has a `TURNSTILE_SECRET` (and `TURNSTILE_SITEKEY`), the sign-up form shows a Turnstile "I'm human" check and the server verifies its token with Cloudflare before creating the account — this replaces the proof-of-work puzzle. Without those variables it stays on proof-of-work, so nothing changes until you add the keys. To turn it on: make a **Turnstile** widget in the Cloudflare dashboard (Turnstile → Add site, domain `case-sim.pages.dev`), then in the Pages project's **Settings → Variables** add `TURNSTILE_SITEKEY` (the public site key) and `TURNSTILE_SECRET` (the secret key, as an encrypted variable). If Cloudflare's check is ever unreachable, remove those two variables to fall back to proof-of-work.
...
- **Battles.** Entry is paid on joining. When the last seat fills, or the creator starts early and bots take the empty seats, the server picks a seed, rolls the whole battle and pays the winner. Every player's game replays the same rolls from that seed, starting at the same moment. Each round, every player's reel spins like opening a case and lands on their roll.
...
- **The Shop and keyed crates.** Haunted Hollow (Halloween, Oct 1 – Nov 2), Harvest Crate (Thanksgiving, Nov 1 – 30), Winter Wonder (Christmas, Dec 1 – Jan 6) and the all-year Launch Crate open with a crate and a matching key, both bought in the **Shop** tab (`POST /api/shop`, up to 10 at once). Crates and keys are ordinary inventory items, so they can be traded, listed on the market or sold for what they cost. Holiday crates and their keys are only sold in their season (the Shop hides them otherwise), but a crate and key you already hold open any time. A crate and its key together cost what the case used to (666, 450, 1,225 and 500 coins), and the odds are tuned on that total. Keyed cases aren't used in battles. The admin panel's **Game** tab can switch a season on early or off. The **Frosty Present** can't be bought: presents come from rewards, gifts, admin gifts and Winter Wonder cases (a 2% bonus drop), open without a key at any time of year, and show on the case page while you hold one.
- **2026 tag.** Every item from the holiday cases, their crates and keys, and the Frosty Present shows a blue **2026** tag.
...
- **Owner sign-in history** (bottom of Overview): every network address that signed in to the owner account (`ADMIN_ACCOUNTS` in `wrangler.toml`), got its password wrong, or used an already-signed-in session, with the device, counts, and first and last seen. Only the admin key or the owner account itself can see it; other admins get nothing.
...
| POST | `/api/shop` | ✓ | `{ case_id, what: 'crate' \| 'key', count 1-10 }` |
| POST | `/api/sell` | ✓ | `{ ids }` |
