...
ok('...but not the owner\'s sign-in history', (await acctAdmin(mt, { a: 'owner_access' })).status === 403);
r = await admin({ a: 'owner_access' });
ok('the admin key reads the owner\'s sign-in history', r.status === 200 && Array.isArray(r.data.rows), r.data);
